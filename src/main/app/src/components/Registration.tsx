...
function Registration() {
  ...
  // placeOfVolunteering
  const [placeOfVolunteering, setPlaceOfVolunteering] = React.useState({
    id: undefined,
  });
  const [
    errorPlaceOfVolunteering,
    setErrorPlaceOfVolunteering,
  ] = React.useState(false);
  const [
    helperTextPlaceOfVolunteering,
    setHelperTextPlaceOfVolunteering,
  ] = React.useState("");

  // qualifications
  const [qualificationSelect, setQualificationSelect] = React.useState(
    emptyGroup
  );
    ...
    request("places", (placesResponse: any) => {
      ...
      placesResponse.json().then((placesData: any) => {
        const places = placesData.map((place: any) => ({
          ...
          label: `${place.name}, ${place.postCode}, ${place.county}`,
          group: place.county,
        }));
    ...
    request("qualifications", (qualificationsResponse: any) => {
      ...
      qualificationsResponse.json().then((qualificationsData: any) => {
        const qualifications = qualificationsData.map((qualification: any) => ({
          ...
          group: qualification.qualificationGroup
            ? qualification.qualificationGroup.name
            : "",
          orderNum: qualification.orderNum,
          groupOrderNum: qualification.qualificationGroup
            ? qualification.qualificationGroup.orderNum
            : undefined,
        }));
        const sortedQualifications = sortData(
          qualifications,
          defaultDataGroupSort
        );
    ...
    request("experiences", (experiencesResponse: any) => {
      ...
      experiencesResponse.json().then((experiencesData: any) => {
        const experiences = experiencesData.map((experience: any) => ({
          ...
          group: experience.experienceGroup
            ? experience.experienceGroup.name
            : "",
          orderNum: experience.orderNum,
          groupOrderNum: experience.experienceGroup
            ? experience.experienceGroup.orderNum
            : undefined,
        }));
    ...
    request("skills", (skillsResponse: any) => {
      ...
      skillsResponse.json().then((skillsData: any) => {
        const skills = skillsData.map((skill: any) => ({
          ...
          groupOrderNum: skill.skillGroup
            ? skill.skillGroup.orderNum
            : undefined,
        }));
  ...
  const validatingFields = (form: any, data: any): boolean => {
    ...
    if (!placeOfVolunteering.id) {
      ...
      setHelperTextPlaceOfLiving(
        "nije odabrano mjesto u kojem želite volontirati"
      );
      ...
      !!options.find(
        (category) =>
          ...
          category.options.find(
            (item: ListItem) =>
              item && item.label && item.label.toLocaleLowerCase() === search
          )
      )
    );
  };
    ...
    <div className="App">
      <header className="App-header">
        <div className="ctnRegister">
          <form
            onSubmit={onSubmit}
            action="http://localhost:8080/api/v1/volunteers"
            target="_self"
            autoComplete="off"
          >
            <fieldset className="fieldset">
              ...
              <FormControl error={errorDOB}>
                <FormControlLabel
                  ...
                  control={
                    <Datetime
                      viewMode="years"
                      viewDate={dateTimeValue}
                      dateFormat={"DD.MM.YYYY"}
                      open={dateTimeOpened}
                      onChange={onChangeDateTime}
                      closeOnSelect={true}
                      className="textField rdt-datepicker"
              ...
              <FormControlLabel
                control={
                  <TextField
                    ...
                    inputProps={{
                      onBlur: (event) =>
                        validateOIB(event.target, event.target.value),
                      minLength: 11,
                      maxLength: 11,
                    }}
                    className="textField"
              ...
              >
                <FormControlLabel
                  ...
                  label="Mjesto u kojem želite volontirati*:"
                  className="textField"
                  labelPlacement="top"
                />
                <FormHelperText className="helper-text">
            ...
            <fieldset className="fieldset">
              ...
              <div className="fieldset-info">
                Kako bi brže i učinkovitije rasporedili volontere na
                odgovarajuće volonterske pozicije i najbolje iskoristili resurse
                kojima raspolažemo, molimo da odgovorite na dodatnih nekoliko
                pitanja.
              </div>
                ...
                <FormControlLabel
                  control={
                    <CreatableSelect
                      ...
                      isValidNewOption={(search) =>
                        !includes(search, qualificationSelect)
                      }
                      formatGroupLabel={formatGroupLabel}
                      formatCreateLabel={(option) => `Dodaj: "${option}"`}
                      noOptionsMessage={noOptionsMessage}
                      isMulti
                    />
                  }
                  label="Zanimanje/profesionalne kvalifikacije*:"
                  className="textField"
                  labelPlacement="top"
                />
              </FormControl>
              ...
              <FormControl className="fullWidth">
                <FormControlLabel
                  control={
                    <CreatableSelect
                      ...
                      closeMenuOnSelect={false}
                      isValidNewOption={(search) =>
                        !includes(search, experienceSelect)
                      }
                      formatGroupLabel={formatGroupLabel}
                      formatCreateLabel={(option) => `Dodaj: "${option}"`}
                      noOptionsMessage={noOptionsMessage}
                ...
                />
                <FormHelperText className="helper-text">
                  Navedite postojeća iskustva u odgovoru na kriznu situaciju ili
                  pripadnost nekoj drugoj organizaciji
                </FormHelperText>
              </FormControl>
              ...
              <FormControl className="fullWidth">
                ...
                />
                <FormHelperText className="helper-text">
                  Navedite ukoliko želite ponuditi dodatne usluge koje smatrate
                  da mogu doprinijeti radu Crvenog križa (npr. usluge prijevoza,
                  ustupanja vozila i sl.)
                </FormHelperText>
              </FormControl>
            ...
            <fieldset className="fieldset">
              ...
              <FormControlLabel
                ...
                label="Koliko ste sati tjedno spremni izdvojiti na volontiranje*:"
                className="textField"
                labelPlacement="top"
              />
