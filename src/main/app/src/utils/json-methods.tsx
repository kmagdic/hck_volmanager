...
export const deepField = (obj: any, field: string) =>
  field
    .split(".")
    .reduce(
      (att: any, value: any) => (att && att[value] ? att[value] : null),
      obj
    );
...
export const defaultDataGroupSort: any[] = [
  (a: any, b: any) =>
    toSafeNumber(a.groupOrderNum) - toSafeNumber(b.groupOrderNum),
  "group",
  (a: any, b: any) => {
...
export const groupingOptions = (
  ...
  data.forEach((option) => {
    ...
    if (groupedObject[option.group]) {
      ...
      groupedObject[option.group] = {
        label: option.group || emptyGroup,
        options: [item],
      };
    }
  });
...
export const join = (first: any, second: any, create: any) =>
  first.map(
    (fItem: any) => second.find((sItem: any) => create(fItem, sItem)) || fItem
  );
